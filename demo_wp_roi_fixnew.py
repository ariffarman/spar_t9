...
##################### changed tolerance ##########################
	...
	## Potential fix - replace existing part
	def send_wp(self, wp, require_convergence=True):
    		...
    		goal.position.x, goal.position.y, goal.position.z = wp[0:3]
    		goal.velocity_vertical = self.vel_linear
    		goal.velocity_horizontal = self.vel_linear
    		goal.wait_for_convergence = require_convergence
					...
					if not self.breadcrumbMode:
						...
						if len(res.path.poses) > 0:
							...
							self.waypoint_counter += 1
        						...
        						is_last = (self.breadcrumbWPSnextIndex == len(self.breadcrumbWPS) - 1)
        						# Disable convergence for intermediate points (fly-through)
        						self.send_wp(self.breadcrumbWPS[self.breadcrumbWPSnextIndex],
                     						     require_convergence=is_last)
        						self.breadcrumbWPSnextIndex += 1
						    else:
...
def main(args):
	...
	surveyAltitude = rospy.get_param("~surveyAlt", 2.5)
	#surveyAltitude = 2
	# List of waypoints
	#surveyAltitude = 1.8
	# [X, Y, Z, Yaw]
